...
static char MapBitToPromo(uint64_t bit, const Board& brd)
{
    if (bit & brd.BPawn)    return 'p';
    if (bit & brd.BKnight)  return 'n';
    if (bit & brd.BBishop)  return 'b';
...
static char MapBitToRank(uint64_t bit)
{
    if (bit < 8)    return '8';
    if (bit < 16)    return '7';
    if (bit < 24)    return '6';
...
static char MapBitToFile(uint64_t bit)
{
    bit = bit % 8;
    if (bit == 0)    return 'a';
    if (bit == 1)    return 'b';
...
static std::string _map_move(uint64_t val1, uint64_t val2)
{
    static std::string str(4, 'o'); //4 letter uci notation
    ...
    for (uint64_t i = 0, c = 0; i < 64; i++)
    {
        ...
        if ((bitmask & val1) != 0){
		// From square, but if castling, str will get filled here all 4 slots by the king move
		str[0] = MapBitToFile(c);
		str[1] = MapBitToRank(c);
	}
        if ((bitmask & val2) != 0) {
		// To square
		str[2] = MapBitToFile(c);
		str[3] = MapBitToRank(c);
	}
...
static std::string _map_castle(uint64_t val1)
{
    static std::string str(4, 'o'); //4 letter uci notation
    uint64_t wkbitmask = (1ull << 63) >> 4;
        if ((wkbitmask & val1) != 0){
		str[0] = 'e';
		str[1] = '8';
                uint64_t wkqscbitmask = (1ull << 63) >> 2;
	       	if ((wkqscbitmask & val1) != 0){
		  str[2] = 'c';
		  str[3] = '8';
	 	} else {
		  str[2] = 'g';
          ...
          uint64_t bkbitmask = (1ull << 63) >> 60;
          if ((bkbitmask & val1) != 0){
		str[0] = 'e';
		str[1] = '1';
                uint64_t bkqscbitmask = (1ull << 63) >> 62;
	       	if ((bkqscbitmask & val1) != 0){
		  str[2] = 'g';
		  str[3] = '1';
	 	} else {
		  str[2] = 'c';
